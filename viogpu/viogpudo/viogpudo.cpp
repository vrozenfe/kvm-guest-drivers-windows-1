                         ...
                         _Out_ ULONG*             pNumberOfChildren)
{
    ...
    else
    {
        ...
        DbgPrint(TRACE_LEVEL_ERROR, ("StartDevice Invalid hardware\n"));
        return Status;
    }
#endif
    if (!m_pHWDevice)
    ...
    Status = RegisterHWInfo(m_pHWDevice->GetInstanceId());
    if (!NT_SUCCESS(Status))
    {
...
//            VioGpuDbgBreak();
            ...
            pDriverCaps->SupportNonVGA = TRUE;//(m_pHWDevice->GetType() == VGA_DEVICE);
...
{
    return m_bEDID ? m_EDIDs[Id] : NULL;
}
...
//FIXME validate size properly
        if (pa.QuadPart == 0) {
            m_bPrimary = FALSE;
        }
        if (pa.QuadPart == 0 || sz < 2048 * 2048 * 4) {
            pa.QuadPart = 0;
            sz = 2048 * 2048 * 4;
        }
        if (!m_FrameSegment.Init(sz, pa.QuadPart ? &pa : NULL))
        {
...
//FIXME rotation
    ...
    m_CtrlQueue.TransferToHost2D(resid, 0UL, pCurrentBddMod->DispInfo.Width, pCurrentBddMod->DispInfo.Height, 0, 0, NULL);
    m_CtrlQueue.ResFlush(resid, pCurrentBddMod->DispInfo.Width, pCurrentBddMod->DispInfo.Height, 0, 0);
...
BOOLEAN VioGpuAdapter::InterruptRoutine(_In_ PDXGKRNL_INTERFACE pDxgkInterface, _In_  ULONG MessageNumber)
{
    ...
    {
        DbgPrint(TRACE_LEVEL_INFORMATION, ("---> %s MSI MessageNumber = %d\n", __FUNCTION__, MessageNumber));
        if (MessageNumber == 1) {
            InterlockedOr((PLONG)&m_PendingWorks, ISR_REASON_DISPLAY);
        }
        else if (MessageNumber == 2) {
            InterlockedOr((PLONG)&m_PendingWorks, ISR_REASON_CURSOR);
        }
    ...
    if (serviced && !pDxgkInterface->DxgkCbQueueDpc(pDxgkInterface->DeviceHandle)) {
